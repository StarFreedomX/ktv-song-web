...
3. 进入房间后，打开「添加新歌曲」，搜索或粘贴 B 站分享内容来点歌。提交后窗口立即关闭，稍后会提示添加成功或失败；添加成功的歌曲显示到歌单末尾。
4. 在房间中查看歌单、调整待唱顺序，并在唱完后切换到下一首。

已被使用的房间号不能重复创建，可以换一个房间号，或加入已有房间。

如果分享链接提示「房间不存在或已失效」，可以点击「创建房间」，使用链接中的房间号创建并进入新房间，也可以返回首页创建或加入其他房间。房间默认在停止更新歌单约 24 小时后失效，具体时长由部署者设置；查看历史记录不会延长房间有效期。

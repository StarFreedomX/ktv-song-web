...
export default {
    ...
    setup() {
        ...
        const isAddingToFavorites = ref(false); // 标记当前弹窗是否为“添加收藏”
        ...
        watch(() => historyList.value[historyList.value.length - 1], (newSong, oldSong) => {
            ...
            if (hostMode.value && newSong && (!oldSong || newSong.id !== oldSong.id)) {
                console.log('主机模式：检测到新歌，正在自动跳转...', newSong.title);
        ...
        const addFavoriteToQueue = async (fav) => {
            ...
            const success = await commitOp({
                song: newSong, toIndex: effectiveLen
            });
        ...
        const commitOp = async (opData) => {
            ...
            try {
                ...
                if (res.success) {
                    if (await getHash(songs.value) !== res.hash) await load(); else lastHash.value = res.hash;
